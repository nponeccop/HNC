...
data P = P (Map [Char] T, Map [Char] T, T) | N [Char]
...
check (CNum n) et = P (M.empty, M.empty, T "num")
check (CBool n) et = P (M.empty, M.empty, T "bool")
check (CStr n) et = P (M.empty, M.empty, T "str")
check (CVal n) et =
	case M.lookup n et of
		Just a -> P (M.empty, M.empty, a)
		Nothing -> N $ (++) "check cannot find " $ show n

check (CL a (K p)) et =
	case check a et of
		P (lm, rm, TT p1)|M.null lm && M.null rm ->
...
check (CL a (S p)) et =
	case check a (putp p (take (length p) $ repeat (TU "a")) et) of
		P (ul, ur, ts) -> P (M.empty, M.empty, TT $ (Prelude.map (\t -> t) $ M.elems ur)++[ts])
		o -> o
