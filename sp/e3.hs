...
data Eval = Enum Int | En [Char] | Eb Bool
	| Efun Eval [Eval] | Elist [Eval]
	deriving Show

call_fun (Efun (En "incr_fun") (Enum n:[])) = Enum (n + 1)
call_fun (Efun (Efun f p2) p1) = Efun f (p2++p1)

data Context = Context (Map [Char] Eval)
base = Context (M.fromList [
	("one", Enum 1),
	("incr", Efun (En "incr_fun") [])])
...
eval (Sfun f p) c =
	call_fun (Efun (eval f c) (Prelude.map (\x -> eval x c) p))
